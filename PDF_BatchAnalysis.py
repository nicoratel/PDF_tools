...
class StructureGenerator():
    def __init__(self,pdfpath,cif_file:str,size_array:tuple, min_params:tuple=[1,0],max_params:tuple=[10,10],sphere_only: bool=False):
        """
    ...
    def makeSphere(self,phi):
        ...
        #write xyz file
        cifname=(os.path.basename(self.cif_file).split('/')[-1]).split('.')[0]
        filename=f'Sphere_phi={int(phi)}_{cifname}'
    ...
    def makeIcosahedron(self,p):
        ...
        cifname=(os.path.basename(self.cif_file).split('/')[-1]).split('.')[0]
        filename=f'Ih_{p}shells_phi={int(2*self.diameter_from_Atoms(ico))}_{cifname}'
        self.writexyz(filename,ico)
        return filename,2*self.diameter_from_Atoms(ico)
    
    def makeDecahedron(self,p,q):
        deca=Decahedron(self.atoms[0],p,q,0,self.a)
        cifname=(os.path.basename(self.cif_file).split('/')[-1]).split('.')[0]
        filename=f'Dh_{p}_{q}_phi={int(2*self.diameter_from_Atoms(deca))}_{cifname}'
        self.writexyz(filename,deca)
        return filename,2*self.diameter_from_Atoms(deca)
    ...
    def makeOctahedron(self,p,q):
        ...
        cifname=(os.path.basename(self.cif_file).split('/')[-1]).split('.')[0]
        if q==0:
            filename=f'RegOh_{p}_0_phi={int(2*self.diameter_from_Atoms(octa))}_{cifname}'
        if p==2*q+1:
            filename=f'CubOh_{p}_{q}_phi={int(2*self.diameter_from_Atoms(octa))}_{cifname}'
        if p==3*q+1:
            filename=f'RegTrOh_{p}_{q}_phi={int(2*self.diameter_from_Atoms(octa))}_{cifname}'
        else:
            filename=f'TrOh_{p}_{q}_phi={int(2*self.diameter_from_Atoms(octa))}_{cifname}'
        self.writexyz(filename,octa)
        return filename,2*self.diameter_from_Atoms(octa)
    ...
    def run(self):
        ...
        line2write+='*****************************************************\n\n'
        line2write+='Structure File                Diameter of envelopping sphere\n'
        print(line2write)
        if not self.sphere_only:
            ...
            for p in p_array:
                filename,size=self.makeIcosahedron(p)
                print(f'{filename:30}{size}')
                line2write+=f'{filename:30}{size}\n'
                for q in q_array:
                    filename,size=self.makeDecahedron(p,q)
                    print(f'{filename:30}{size}')
                    line2write+=f'{filename:30}{size}\n'
                    if q<=(p-1)/2:
                        filename,size=self.makeOctahedron(p,q)
                        print(f'{filename:30}{size}')
                        line2write+=f'{filename:30}{size}\n'
            for size in self.size_array:
                filename,size=self.makeSphere(size)
                print(f'{filename:30}{size}')
                line2write+=f'{filename:30}{size}\n'
        else:
            for size in self.size_array:
...
class StructureCustom():
    def __init__ (self, 
        ...
        fraction: float, fraction of the newu element (between 0 and 1)
        """
...
class PDFRefinement():
    def __init__(self,
                 ...
                 plot_tag: save refinement data (default=False)
                 RUN_PARALLEL=True
    ...
    def run(self):
        ...
        for pdffile in pdffile_list:
            ...
            for strufile in strufile_list:
                ...
                rw=calc.refine()
                temp=f'{pdfname:30}\t{struname:30}\t{rw:.4f}'
                print(temp)
                line2write+=f'{pdfname:30}\t{struname:30}\t{rw:.4f}\n'
                rw_array[j,i]=rw
                i+=1
